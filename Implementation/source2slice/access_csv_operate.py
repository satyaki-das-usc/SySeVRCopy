...
def read_edges_paths_from_file():
    # Load the data from 'edges_csv_file_paths.json'
    with open('edges_csv_file_paths.json', 'r') as edges_json_file:
        edges_data = json.load(edges_json_file)

    return edges_data

def read_nodes_paths_from_file():
    # Load the data from 'nodes_csv_file_paths.json'
    with open('nodes_csv_file_paths.json', 'r') as nodes_json_file:
        nodes_data = json.load(nodes_json_file)

    return nodes_data

def read_nodes_data(node_csv_files):
    node_data = []
    for file_path in node_csv_files:
...
def read_edges_data(edge_csv_files):
    edge_data = []
    for file_path in edge_csv_files:
        with open(file_path, newline='') as csvfile:
            csv_reader = csv.DictReader(csvfile, delimiter='\t')
            for row in csv_reader:
...
# def get_all_use_bydefnode(node_id):
#     # query_str = "g.v(%d).in('USE')" % node_id
#     # results = db.runGremlinQuery(query_str)
#     node_in_edge_keys = []
#     for file_path in edges_csv_files:
#         with open(file_path, newline='') as csvfile:
#             csv_reader = csv.DictReader(csvfile,delimiter='\t')
#             for row in csv_reader:
#                 if (row['end']== node_id and row["type"] == 'USE'):
#                     node_in_edge_keys.append(row["start"] )
#     results = []
#     for file_path in nodes_csv_files:
#         with open(file_path, newline='') as csvfile:
#             csv_reader = csv.DictReader(csvfile,delimiter='\t')
#             for row in csv_reader:
#                 if (row["key"] in node_in_edge_keys):
#                     results.append(row)
                    
#     list_re = []
#     for re in results:
#         if re['type'] == 'Statement':
#             continue
#         else:
#             list_re.append(re)

#     return list_re
# get_all_use_bydefnode('1515')
...
#     results = db.runGremlinQuery(query_str)
#     return results
def getALLFuncNode(nodes_csv_files):
    #query_str = "queryNodeIndex('type:Function')"
    results = []
    for file_path in nodes_csv_files:
        with open(file_path, newline='') as csvfile:
...
#     print(ret)
#     return ret[0]
def getFuncFile(func_id):
    edges_csv_files = read_edges_paths_from_file()
    nodes_csv_files  = read_nodes_paths_from_file()
    ...
    for file_path in edges_csv_files:
        with open(file_path, newline='') as csvfile:
            csv_reader = csv.DictReader(csvfile,delimiter='\t')
            for row in csv_reader:
                if (row['end']== func_id and row["type"] == 'IS_FILE_OF'):
                    node_in_edge_keys.append(row["start"] )
    results = []
    for file_path in nodes_csv_files:
        with open(file_path, newline='') as csvfile:
...
def getCFGEdges(func_id):
    ...
    for file_path in nodes_csv_files:
        with open(file_path, newline='') as csvfile:
            csv_reader = csv.DictReader(csvfile,delimiter='\t')
            for row in csv_reader:
                if (row["functionId"] == func_id and row["isCFGNode"] == 'True'):
                    node_keys.append(row["key"] )
    cfgEdges = []
    for file_path in edges_csv_files:
        with open(file_path, newline='') as csvfile:
            csv_reader = csv.DictReader(csvfile,delimiter='\t')
            for row in csv_reader:
                if (row["start"] in node_keys and row["type"] == 'FLOWS_TO'):
                    cfgEdges.append(row)
    return cfgEdges
#getCFGEdges('1457')

def create_edge_objects(edges):
    node_csv_files = read_nodes_paths_from_file()
    node_data = read_nodes_data(node_csv_files)
    edge_objects = []

    for edge in edges:
        edge_object = Edge()
        for row in node_data:
            if (row['key']== edge['start']):
               edge_object.start_node = row
               edge_object.self = edge
            elif(row['key']== edge['end']):
               edge_object.end_node = row
...
def drawGraph(edges, func_entry_node, graph_type):
    g = Graph(directed=True)
    func_id = func_entry_node['key']
    filepath = getFuncFile(func_id)

    edge_objects = create_edge_objects(edges)
    for edge in edge_objects:
        if edge.start_node['code'] == 'ENTRY':
            startNode = str(edge.start_node['functionId'])
        else:
            startNode = str(edge.start_node['key'])

        if edge.start_node['code'] == 'ERROR':
            continue

        if isNodeExist(g, startNode) == False:
            if edge.start_node['code'] == 'ENTRY':
                node_prop = {'code': func_entry_node['code'], 'type': func_entry_node['type'],
                         'location': func_entry_node['location'], 'filepath':filepath, 'functionId':str(edge.start_node['functionId'])}
            else:
                node_prop = {'code': edge.start_node['code'], 'type': edge.start_node['type'],
                         'location': edge.start_node['location'], 'filepath':filepath, 'functionId':str(edge.start_node['functionId'])}
            g.add_vertex(startNode, **node_prop)#id is 'name'

        endNode = str(edge.end_node['key'])
        if isNodeExist(g, endNode) == False:
            if graph_type == 'pdg' and edge.end_node['code'] == 'EXIT':
                continue

            if edge.end_node['code'] == 'ERROR':
                continue

            node_prop = {'code': edge.end_node['code'], 'type': edge.end_node['type'],
                         'location': edge.end_node['location'], 'filepath':filepath, 'functionId':str(edge.end_node['functionId'])}
            g.add_vertex(endNode, **node_prop)

        if graph_type == 'pdg':
            edge_prop = {'var': edge['var']}
        else:
            #if it is cfg then type is flowLabel
...
def translateCFGByNode(func_node):
    func_id = func_node['key']
    Edges = getCFGEdges(func_id)
    graph_type = 'cfg'
    g = drawGraph(Edges, func_node, graph_type)
...
def get_all_calls_node(db, testID):
    ...
    print("length", len(list_all_funcID))
