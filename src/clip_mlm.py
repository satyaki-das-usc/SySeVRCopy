...
class CLIP(nn.Module):
    ...
    def forward(
        ...
        if training_classifier:
            ...
            enc_text1 = l2norm(self.to_text_latent1(enc_text1))
            enc_text1 = self.batch_norm(self.to_text_latent2(enc_text1))

            enc_text = l2norm(self.fc1(enc_text.mean(dim=1)))
            enc_text = self.batch_norm(self.fc2(enc_text))

            return self.fc3(torch.cat([enc_text, enc_text1], dim=-1))
